<?php
$_REQUEST['user_name']= isset($_REQUEST['user_name']) ? $_REQUEST['user_name'] : $_SESSION['user'];
$_REQUEST['set_password']= isset($_REQUEST['set_password']) ? $_REQUEST['set_password'] : retrieve_password($dbconn, $_SESSION['user']);
$_REQUEST['confirm_password']= isset($_REQUEST['confirm_password']) ? $_REQUEST['confirm_password'] : '';
$_REQUEST['email']= isset($_REQUEST['email']) ? $_REQUEST['email'] : retrieve_email($dbconn, $_SESSION['user']);
$_REQUEST['coffee']= isset($_REQUEST['coffee']) ? $_REQUEST['coffee'] : retrieve_coffee($dbconn, $_SESSION['user']);
$_REQUEST['game']= isset($_REQUEST['game']) ? $_REQUEST['game'] : retrieve_game($dbconn, $_SESSION['user']);
$_REQUEST['weekday']= isset($_REQUEST['weekday']) ? $_REQUEST['weekday'] : retrieve_weekday($dbconn, $_SESSION['user']);
?>
...
<html lang="en">
	...
	<body>
		...
		<main>
			<section>
				<h1>Check And Edit Your Profile Here testing</h1>
				<form id="registrationForm" method="post">
					...
					<div class="form-row" id="r8">
						<button type="submit" name="change_profile" value="submitted">Make Changes (you will be logged out)</button>
					</div>
				</form>
